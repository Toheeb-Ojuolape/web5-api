...
var require_lib = __commonJS({
  "node_modules/@decentralized-identity/ion-sdk/node_modules/@noble/ed25519/lib/index.js"(exports) {
    ...
    async function sign4(message, privateKey) {
    ...
    exports.sign = sign4;
    function signSync2(message, privateKey) {
      message = ensureBytes3(message);
    ...
    async function verify4(sig, message, publicKey) {
    ...
    async function getSharedSecret3(privateKey, publicKey) {
    ...
    exports.getSharedSecret = getSharedSecret3;
    ...
    function montgomeryLadder2(pointU, scalar) {
      const { P } = CURVE3;
    ...
    function encodeUCoordinate2(u) {
      return numberTo32BytesLE2(mod3(u, CURVE3.P));
    }
    function decodeUCoordinate2(uEnc) {
    ...
    exports.curve25519 = {
      ...
      scalarMult(privateKey, publicKey) {
        const u = decodeUCoordinate2(publicKey);
        const p = decodeScalar255192(privateKey);
        const pu = montgomeryLadder2(u, p);
        if (pu === _0n3)
          throw new Error("Invalid private or public key received");
        return encodeUCoordinate2(pu);
      },
...
var require_lib2 = __commonJS({
  "node_modules/@decentralized-identity/ion-sdk/node_modules/@noble/secp256k1/lib/index.js"(exports) {
    ...
    exports.getPublicKey = getPublicKey3;
    function recoverPublicKey2(msgHash, signature, recovery, isCompressed = false) {
    ...
    function getSharedSecret3(privateA, publicB, isCompressed = false) {
    ...
    exports.getSharedSecret = getSharedSecret3;
    function bits2int2(bytes) {
      const slice = bytes.length > 32 ? bytes.slice(0, 32) : bytes;
      return bytesToNumber2(slice);
    }
    function bits2octets2(bytes) {
      const z1 = bits2int2(bytes);
    ...
    var SchnorrSignature2 = class {
      ...
      static fromHex(hex) {
        ...
        return new SchnorrSignature2(r, s);
      }
      assertValidity() {
        const { r, s } = this;
    ...
    var InternalSchnorrSignature = class {
      ...
      finalizeSig(R, k, e, d) {
        return new SchnorrSignature2(R.x, mod3(k + e * d, CURVE3.n)).toRawBytes();
      }
      error() {
    ...
    function initSchnorrVerify(signature, message, publicKey) {
      const raw = signature instanceof SchnorrSignature2;
      const sig = raw ? signature : SchnorrSignature2.fromHex(signature);
      if (raw)
        sig.assertValidity();
      return {
        ...sig,
    ...
    async function schnorrVerify2(signature, message, publicKey) {
      try {
        ...
        const e = schnorrChallengeFinalize(await exports.utils.taggedHash(TAGS.challenge, numTo32b2(r), P.toRawX(), m));
    ...
    function schnorrVerifySync(signature, message, publicKey) {
      try {
        const { r, s, m, P } = initSchnorrVerify(signature, message, publicKey);
        const e = schnorrChallengeFinalize(exports.utils.taggedHashSync(TAGS.challenge, numTo32b2(r), P.toRawX(), m));
        return finalizeSchnorrVerify(r, P, s, e);
      } catch (error) {
...
async function taggedHash(tag, ...messages) {
  ...
  const tagH = await utils2.sha256(tagB);
  const h = await utils2.sha256(concatBytes2(tagH, tagH, ...messages));
  return bytesToNumber(h);
}
async function createChallenge(x, P, message) {
  const rx = numTo32b(x);
  const t = await taggedHash("BIP0340/challenge", rx, P.toRawX(), message);
  return mod2(t, CURVE2.n);
}
function hasEvenY(point) {
  return (point.y & _1n2) === _0n2;
}
...
async function schnorrSign(message, privateKey, auxRand = utils2.randomBytes()) {
  if (message == null)
    throw new TypeError(`sign: Expected valid message, not "${message}"`);
  const { n } = CURVE2;
  const m = ensureBytes2(message);
  const d0 = normalizePrivateKey(privateKey);
  const rand = ensureBytes2(auxRand);
  if (rand.length !== 32)
    throw new TypeError("sign: Expected 32 bytes of aux randomness");
  const P = Point2.fromPrivateKey(d0);
  ...
  if (k0 === _0n2)
    throw new Error("sign: Creation of signature failed. k is zero");
  const R = Point2.fromPrivateKey(k0);
...
async function schnorrVerify(signature, message, publicKey) {
  ...
  const R = Point2.BASE.multiplyAndAddUnsafe(P, normalizePrivateKey(s), mod2(-e, CURVE2.n));
...
var utils2 = {
  ...
  hashToPrivateKey: (hash) => {
    ...
    if (hash.length < 40 || hash.length > 1024)
      throw new Error("Expected 40-1024 bytes of private key as per FIPS 186");
    const num = mod2(bytesToNumber(hash), CURVE2.n);
    if (num === _0n2 || num === _1n2)
      throw new Error("Invalid private key");
    return numTo32b(num);
  },
  randomBytes: (bytesLength = 32) => {
  ...
  sha256: async (message) => {
    if (crypto3.web) {
      const buffer = await crypto3.web.subtle.digest("SHA-256", message.buffer);
      return new Uint8Array(buffer);
    } else if (crypto3.node) {
...
async function verify3(params = {}) {
  ...
  switch (publicJwk.crv) {
    case "secp256k1": {
      ...
      return verify2(signatureBytes, hashedMessage, publicKeyBytes);
    }
    case "Ed25519": {
